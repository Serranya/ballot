...
@MountPath("/ballots/new/setChoices")
public class CreateBallotPage extends BasePage<Void> {
	...
	@SpringBean
	private BallotCreationService ballotCreationService;

	public CreateBallotPage() {
		throw new RestartResponseException(getApplication().getHomePage());
	}

	public CreateBallotPage(IModel<String> name) {
		this(name, new ListModel<>(new ArrayList<>()));
	}

	public CreateBallotPage(IModel<String> name, IModel<? extends List<? extends Choice>> choices) {
		this.name = name;
		this.choices = new ListModel<>(choices.getObject().stream()
				.map(c -> ModifiableChoice.create(c.getDisplayValue())).collect(Collectors.toList()));
	}
	...
	protected void onInitialize() {
		...
		add(new Form<>("form") {
			...
			protected void onInitialize() {
				...
				CreateBallotPage.this.addNewChoiceComponents(this);
			}

			protected void onError() {
				CreateBallotPage.this.onError();
			};
		});
		add(newDone("done"));
		...
		return new AjaxLink<>(id) {
			public void onClick(AjaxRequestTarget target) {
				if (choices.getObject().size() <2) {
					...
					CreateBallotPage.this.onDone(name, choices);
				}
			};
		};
	};
	...
	protected void onDone(IModel<String> name, IModel<? extends Collection<? extends Choice>> choices) {
		...
		ballotCreationService.createBallot(ballot, name.getObject());
